...
exports.addComment = exports.upvoteIdea = exports.createIdea = exports.getIdeas = void 0;
...
const createIdea = async (req, res, next) => {
    try {
        ...
        const user = await prisma_1.prisma.user.findUnique({ where: { id: userId }, include: { department: true } });
        const idea = await prisma_1.prisma.innovationIdea.create({
            data: {
                title,
                description,
...
const upvoteIdea = async (req, res, next) => {
    try {
        const { id } = req.params;
        const idea = await prisma_1.prisma.innovationIdea.update({
            where: { id },
...
exports.upvoteIdea = upvoteIdea;
const addComment = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { content } = req.body;
        const userId = req.user.id;
        const comment = await prisma_1.prisma.ideaComment.create({
            data: {
                content,
                ideaId: id,
                userId
