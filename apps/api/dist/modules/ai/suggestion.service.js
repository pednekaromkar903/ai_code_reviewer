...
class SuggestionService {
    async getSuggestions(userId, role) {
        ...
        // Manager suggestions
        if (role === 'MANAGER') {
            const pendingApprovals = await prisma_1.prisma.goal.count({
            ...
            const overdueCheckins = await prisma_1.prisma.goal.count({
                where: {
        ...
        if (role === 'EMPLOYEE') {
            ...
            const pendingSubmit = await prisma_1.prisma.goal.count({
        ...
        if (role === 'ADMIN') {
            ...
            const totalGoals = await prisma_1.prisma.goal.count();
            const pendingApprovals = await prisma_1.prisma.goal.count({ where: { status: 'PENDING' } });
