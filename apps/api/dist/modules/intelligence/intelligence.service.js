...
const nl_sql_engine_1 = require("./nl-sql.engine");
const engine = new nl_sql_engine_1.NLSQLEngine();
class IntelligenceService {
    ...
    async saveConnection(config) {
        await prisma_1.prisma.dbConnection.updateMany({ data: { isActive: false } });
        const conn = await prisma_1.prisma.dbConnection.create({
            data: {
    ...
    async getActiveConnection() {
        return prisma_1.prisma.dbConnection.findFirst({ where: { isActive: true } });
    }
    getSchema() {
        return {
            tables: [
    ...
    async executeNaturalLanguage(query, role, userId) {
    ...
    async runSafeQuery(q) {
        ...
        if (q.includes('b2b') || q.includes('d2c') || q.includes('revenue') || q.includes('sales')) {
            const rows = await prisma_1.prisma.salesMonthly.findMany({ orderBy: { month: 'asc' }, take: 100 });
            return rows.map((r) => ({
        ...
        const goals = await prisma_1.prisma.legacyGoal.findMany({
            include: { employee: true },
            take: 100
        });
        return goals.map((g) => ({
    ...
    async getLegacyGoals() {
        return prisma_1.prisma.legacyGoal.findMany({
            include: { employee: true },
            orderBy: { title: 'asc' }
        });
    }
